...
> ✅ **已于 2026-08-20 T1.7.1 评审全部拍板**，详见 [评审纪要](./review/T1.7.1-评审纪要.md) / [CHANGE_CONTROL.md](./CHANGE_CONTROL.md) / [DECISIONS.md](../.project-agent/DECISIONS.md)

| # | 事项 | 影响 | 拍板结论 |
|---|------|------|---------|
| 1 | **后端方案选型** | 架构、开发流程、部署方式 | ✅ D-001 腾讯云开发 CloudBase（国内首选） |
| 2 | **AI API 提供商** | 识别准确率、成本、响应速度 | ⏭️ D-008 后置：百度先跑 MVP，app 端到端可测时再定（豆包/Qwen-VL 备选） |
| 3 | **宠物形象设计** | 设计资源、动画制作 | ✅ D-009 水豚 6 状态定稿（静态图+动画已入库） |
| 4 | **推送通知策略** | 用户回访率、活跃度 | ✅ 拍板：MVP 本地通知先行（喂食提醒）；远端推送后续版本，排期仍按 T5.5（P5 增量期） |
| 5 | **AI 兜底策略** | 用户体验 | ✅ 拍板：三级置信度 + Top-K 候选 + 手动输入 |
