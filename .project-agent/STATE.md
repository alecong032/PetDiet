...
- T1.2.3 百度 AI 识别测试脚本 ⬜ ← 障碍：需用户准备 20 张常见中餐测试图片（脚本可先行编写）
- T1.2.4 百度 vs 讯飞对比报告 ⬜ ← 依赖 T1.2.3；讯飞已跳过，范围需用户确认（是否只做百度单方案验证）
- T1.3.1 宠物 emoji 形象候选设计 ⬜ ← 产品设计任务（AI 可出候选，需用户选定）
...
- T1.6.2 Android CI ⬜ ← 依赖 T1.4.2 ✅ 满足，无外部障碍（但按顺序需先处理 T1.2/T1.3/T1.5 障碍）
