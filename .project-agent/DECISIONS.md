...
D-011 基线口径一致性修正（CR-002）：TDD §6.2.4 `map_status` 行按 D-009 六状态
  口径修正（原为旧 5 状态、含已废弃 sad）；评审材料 overfull 括号勘误为
  「摄入 ≥ 110%」；color-system §五 次级文字补明确取值规则（亮色小字用
  §八 onSurfaceVariant #687665，AA 4.81:1）
  理由: 用户拍板（2026-09-16）；三项均为执行遗漏/勘误，不改变功能范围
  影响: P3 实现按 §6.2.4 新口径；设计稿无需改动（已使用 AA 达标值）；
    TDD 仍为冻结基线，本次修改经 CR-002 批准后落地
  日期: 2026-09-16
